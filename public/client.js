...
// var number = parseInt(getQueryVariable("persona"), 10);
    ...
    xhr.onreadystatechange = function(e) {
      if (xhr.readyState === 4) {
        if (xhr.status === 200) {
          ...
          setTimeout(function(){ showItemById('content-container', 1); }, 250); // Show the persona card with a slight delay
        } else {
          reject(xhr.status)
        }
      }
...
//   Constructing the header with data from the "info" sheet
  ...
  contentData.posts.forEach(function(contentItem) {
    
  var title = contentItem.title,
      img = contentItem.img,
      imgalt = contentItem.imgalt,
      id = contentItem.id,
